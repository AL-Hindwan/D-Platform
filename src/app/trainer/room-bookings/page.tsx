    ...
    <div className="mx-auto max-w-7xl space-y-5 bg-slate-50/60 px-4 pb-8 pt-6 sm:px-6" dir="rtl">
                      ...
                      <TableRow key={booking.id} className="h-16">
                        ...
                        <TableCell className="text-right align-middle">
                          <div className="flex flex-col items-start gap-1">
                            ...
                            {booking.rejectionReason ? <div className="max-w-[210px] truncate text-xs text-red-600" title={booking.rejectionReason}>السبب: {booking.rejectionReason}</div> : null}
                          </div>
                        </TableCell>
                          ...
                          <div className="flex flex-wrap items-center justify-start gap-2">
                            ...
                            {booking.status === "REJECTED" && booking.rejectionReason ? (
                              <Button
                                variant="outline"
                                size="sm"
                                className={`h-8 border-red-300 px-3 text-red-700 hover:bg-red-50 ${radiusClass}`}
                                onClick={() => {
                                  setSelectedBooking(booking)
      ...
      >
        <DialogContent dir="rtl" className={`sm:max-w-[425px] border-slate-200 p-6 ${radiusClass}`}>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-right text-red-600">
              <AlertCircle className="h-5 w-5" />
              سبب رفض الطلب
            </DialogTitle>
          </DialogHeader>
          <div className="py-3">
            <p className={`border border-red-100 bg-red-50 p-4 text-right leading-relaxed text-gray-700 ${radiusClass}`}>{selectedBooking?.rejectionReason || "لم يتم توفير سبب الرفض."}</p>
          </div>
          <div className="mt-2 flex justify-end">
            <Button variant="outline" className={radiusClass} onClick={() => setShowRejectionDialog(false)}>إغلاق</Button>
          </div>
        </DialogContent>
      </Dialog>
              ...
              selectedBooking.payments.map((payment: any, index: number) => (
                <div key={payment.id} className={`space-y-2 border p-4 text-sm ${radiusClass} ${index === 0 ? "border-blue-200 bg-blue-50 shadow-sm" : "bg-slate-50"}`}>
                  ...
                  {payment.rejectionReason || selectedBooking.rejectionReason ? (
                    <div className={`mt-2 border-t bg-red-50 p-2 text-red-600 ${radiusClass}`}>
                      <span className="mb-1 block font-medium">سبب الرفض:</span>
                      {payment.rejectionReason || selectedBooking.rejectionReason}
                    </div>
                  ) : null}
                </div>
